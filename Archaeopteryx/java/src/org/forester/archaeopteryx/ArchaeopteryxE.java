...
public class ArchaeopteryxE extends JApplet implements ActionListener {
    ...
    private JCheckBoxMenuItem   _non_lined_up_cladograms_cbmi;
    private Options             _options;
    private JMenuItem           _choose_font_mi;
    private JMenuItem           _switch_colors_mi;
    private JCheckBoxMenuItem   _label_direction_cbmi;
    ...
    public void init() {
        ...
        if ( phys_url != null ) {
            try {
                phys = Util.readPhylogeniesFromUrl( phys_url );
            }
            catch ( final Exception e ) {
    ...
    void updateOptions( final Options options ) {
        ...
        options.setShowScale( ( _show_scale_cbmi != null ) && _show_scale_cbmi.isSelected() );
        ...
        options.setShowBranchLengthValues( ( _show_branch_length_values_cbmi != null )
    ...
    private void openPhyloXmlWebsite() {
        try {
            Util.launchWebBrowser( new URI( Constants.PHYLOXML_WEB_SITE ), true, this, Constants.PRG_NAME );
        }
        catch ( final Exception e ) {
    ...
    private void openWebsite() {
        try {
            Util.launchWebBrowser( new URI( Constants.WEB_SITE ), true, this, Constants.PRG_NAME );
        }
        catch ( final Exception e ) {
            ForesterUtil.printErrorMessage( Constants.PRG_NAME, e.toString() );
        }
