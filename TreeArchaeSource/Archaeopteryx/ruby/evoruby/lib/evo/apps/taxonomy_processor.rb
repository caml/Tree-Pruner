...
#
# $Id: taxonomy_processor.rb,v 1.21 2009/01/03 00:19:08 cmzmasek Exp $
...
module Evoruby
    ...
    class TaxonomyProcessor
        ...
        PRG_NAME       = "tap"
        PRG_DATE       = "2009.01.02"
        PRG_DESC       = "replacment of species names in multiple sequence files"
        PRG_VERSION    = "1.01"
        ...
        def run()
            ...
            if ( ARGV == nil || ( ARGV.length != 4 && ARGV.length != 5 && ARGV.length != 6 ) )
        ...
        def modify_name( desc, counter, file, species_map, extract_taxonomy )
            new_desc = nil
            if desc =~ /^>\s*\S{1,10}_([0-9A-Z]{3,5})[\/|\s]/
